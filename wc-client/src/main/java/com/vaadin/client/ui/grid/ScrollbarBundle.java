...
abstract class ScrollbarBundle {
    ...
    public final double getScrollPos() {
        // FIXME ... happens in hero polymer
        if (internalGetScrollPos() != toInt32(scrollPos)) {
            console.log("MCM ... calculated scroll position ("
                + toInt32(scrollPos)
                + ") did not match the DOM element scroll position ("
                + internalGetScrollPos() + ")" + this.getClass().getName());
