...
public class ApplicationConfiguration implements EntryPoint {
    ...
    public void onModuleLoad() {
        ...
        // and don't continue if the widgetset was not loaded through vaadinBootstrap 
        if (moduleLoaded || !vaadinBootstrapLoaded()) {
            getLogger().log(Level.WARNING, 
                    "vaadinBootstrap.js was not loaded, skiping vaadin application configuration.");
            return;
        }
        moduleLoaded = true;
    ...
    /*-{
        if($wnd.vaadin && $wnd.vaadin.debug) {
            return true;
        } else {
            return false;
        }
