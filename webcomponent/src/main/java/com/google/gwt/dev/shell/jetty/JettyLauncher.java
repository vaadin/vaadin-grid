...
public class JettyLauncher extends ServletContainerLauncher {
    ...
    protected static final class WebAppContextWithReload extends WebAppContext {
        ...
        private class WebAppClassLoaderExtension extends WebAppClassLoader {
            ...
            private final ClasspathPattern allowedFromSystemClassLoader = new ClasspathPattern(
                    new String[] { "org.eclipse.jetty.",
                            ...
                            // Xerces
                            "org.apache.xerces.", "javax.xml.", // Used by Jetty
                                                                // for
                                                                // jetty-web.xml
                                                                // parsing
                    });
            ...
            public URL findResource(String name) {
                ...
                // Note: bootstrap has already been searched, so javax. classes
                // should be
                // tried from the webapp first (except for javax.servlet).
                URL found;
                if (isSystemClass(checkName)
                ...
                if (allowedFromSystemClassLoader.match(checkName)
                        ||
                        // Jasper uses Log4j (via Commons Logging), which will
                        // try
                        // to load those.
                        // We have a log4j.properties in user/test and don't
                        // want
                        // to add gwt-user when using a "Developer SDK" in
                        // Eclipse.
            ...
            protected Class<?> findClass(String name)
                ...
                // Those classes are allowed to be loaded right from the
                // systemClassLoader
                // Note: Jetty classes here are not "server classes", handled
                // above.
        ...
        protected void doStop() throws Exception {
            ...
            Class<?> jdbcUnloader = getClassLoader().loadClass(
                    "com.google.gwt.dev.shell.jetty.JDBCUnloader");
...
//            // Avoid context.getWebInf() == null in
//            // AnnotationConfiguration.parseWebInfClasses
//            wac.setResourceBase(appRootDir.getAbsolutePath());
//            ArrayList<String> confs = new ArrayList<String>(Arrays.asList(wac
